...
function useTokensFromMap(tokenMap: TokenAddressMap, includeUserAdded: boolean, chainId?: ChainId): { [address: string]: Token } {
  ...
  const userAddedTokens = Object.values(useCustomTokens().data ?? ({} as Record<string, Token>))
...
export function useFoundOnInactiveList(searchQuery: string, chainId?: ChainId): Token[] | undefined {
