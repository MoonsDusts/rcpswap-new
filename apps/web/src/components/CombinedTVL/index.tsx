...
export default function CombinedTVL({}) {
  ...
  const isActive = true;
  const filteredStakingInfos = useFilterStakingInfos(
    useStakingInfo(isActive),
    isActive
  );
      ...
      {TVLs?.stakingPoolTVL?.greaterThan("0") && (
            ...
            <>
              ...
              {TVLs.totalPitTVL?.greaterThan("0") && (
                <>
                  ...
                  {TVLs.totalPitTVL.toSignificant(8, { groupSeparator: "," })}
                  <br />
                </>
              )}
        ...
        >
          {TVLs.totalCombinedTVL?.greaterThan("0") && (
            <>
              TVL: $
              {TVLs.totalCombinedTVL.toSignificant(8, { groupSeparator: "," })}
            </>
          )}
        </CustomMouseoverTooltip>
      )}
