...
export default function SwapModalFooter({
  ...
  const { data: price, isInitialLoading: isPriceLoading } = usePrice(
    ...
    wrappedCurrency(fusionSwap.currencies?.OUTPUT, ChainId.ARBITRUM_NOVA)
      ?.address
  );
  ...
  const saving =
    ...
    ethers.BigNumber.from(fusionSwap.result.route?.fee?.amountOutBN ?? "0").gt(
      "0"
    ) &&
    fusionSwap.result.route
      ? parseFloat(
        ...
        {swapMode === 1 &&
          ...
          fusionSwap.result?.route && (
            <RowBetween>
              <RowFixed>
                <TYPE.black color={theme?.text2} fontSize={14} fontWeight={400}>
                  {fusionSwap.result.route.singleProviderRoute?.provider ??
                    "RCP"}
                  Swap Price
                </TYPE.black>
          ...
          saving > 0 && (
            <RowBetween>
              ...
              <TYPE.black fontSize={14}>
                {(
                  saving *
                  (isPriceLoading || !price || price.equalTo("0")
                    ? 1
