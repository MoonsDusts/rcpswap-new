...
export default function AdvancedFusionDetailsDropdown({
      ...
      ? parseFloat(
        ...
        parseFloat(
          new TokenAmount(
            swap.currencies?.OUTPUT as Token,
            ethers.BigNumber.from(
              swap.result.route.fee.amountOutBN ?? "0"
            ).toString()
          ).toExact()
        ) -
        parseFloat(
          new TokenAmount(
            swap.currencies.OUTPUT as Token,
            ethers.BigNumber.from(
              swap.result.route.singleProviderRoute?.amountOutBN ?? "0"
            ).toString()
          ).toExact()
        )
      : 0;
  ...
  return swap.result?.route?.fee?.isFusion && saving > 0 ? (
    <AdvancedDetailsFooter
      show={Boolean(swap && swap?.result && swap?.result?.route)}
    >
      {swap &&
        swap?.result &&
        swap.result?.route &&
              ...
              <AutoColumn style={{ marginLeft: "30px" }}>
                <RowFixed>
                  <Text fontSize={15} color={theme?.green1} fontWeight={600}>
                    {(
                      saving *
                      (isLoading || !price || price.equalTo("0")
                        ? 1
                        : parseFloat(price.toFixed(6)))
                    ).toFixed(3) +
                      " " +
                      (isLoading || !price || price.equalTo("0")
                        ? swap?.currencies?.OUTPUT?.symbol
                        : "$")}
                  </Text>
                ...
                <RowFixed>
                  ...
                  <DexLogo
                    src={
                      require(`../../assets/images/dex/${
                        swap.result.route.singleProviderRoute?.provider?.toLowerCase() ??
                        "rcp"
                      }swap.png`).default
                    }
                  ></DexLogo>
                  <Text fontSize={14} color={theme?.text2}>
                    {swap.result.route.singleProviderRoute?.provider ?? "RCP"}
                    Swap.
                  </Text>
                </RowFixed>
              </AutoColumn>
