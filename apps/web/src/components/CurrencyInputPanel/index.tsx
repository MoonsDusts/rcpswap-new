...
export default function CurrencyInputPanel({
  ...
  let { data: selectedCurrencyBalance } = useBalanceWeb3({
  ...
  const impact =
    inPrice && outPrice
      ? outPrice.totalPrice.greaterThan(0n)
        ? parseFloat(
            inPrice.totalPrice.divide(outPrice.totalPrice).toFixed(6)
          ) *
            100 -
          100
        : 0
          ...
          <PriceRow>
            {!inPrice.loading && inPrice.price?.equalTo("0")
              ...
              : `~$ ${
                  loading
                    ? "0.00"
                    : parseFloat(inPrice.totalPrice.toFixed(2)).toLocaleString(
                        "en-US",
                        {
            ...
            !loading ? (
              <PriceImpact
                impact={loading ? 0 : impact + (saving > 0 ? 1 : 0.3)}
              >
                (
                {loading
                  ? "0.00"
                  : (impact + (saving > 0 ? 1 : 0.3)).toFixed(2)}
                %)
              </PriceImpact>
            ) : null}
          </PriceRow>
